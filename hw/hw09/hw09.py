...
def interleaved_sum(n, odd_term, even_term):
    ...
    "*** YOUR CODE HERE ***"
    def do_odd_term(x):
        return odd_term(x), do_even_term
    def do_even_term(x):
        return even_term(x), do_odd_term
    def loop(x, func):
        if x > n:
            return 0

        val, next = func(x)
        return val + loop(x + 1, next)

    val, next = do_odd_term(1)
    return val + loop(2, next)
...
def mutate_reverse(link):
    ...
    p_link = link
    vals = []
    while p_link is not Link.empty:
        vals.append(p_link.first)
        p_link = p_link.rest
