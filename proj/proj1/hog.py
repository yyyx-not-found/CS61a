...
def roll_dice(num_rolls, dice=six_sided):
    ...
    score = 0
    PIG_OUT = False
    for one_roll in [dice() for _ in range(num_rolls)]:
        if one_roll == 1:
            PIG_OUT = True
        score += one_roll
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    def one_turn(player_strategy, player_score, opponent_score, player_prev_score):
            num_rolls = player_strategy(player_score, opponent_score)
            FERAL_HOGS = (abs(num_rolls - player_prev_score) == 2) and feral_hogs
            obtained_score = take_turn(num_rolls, opponent_score, dice)
            
            player_score += obtained_score
            if FERAL_HOGS: player_score += 3
            player_prev_score = obtained_score

            if is_swap(player_score, opponent_score):
                player_score, opponent_score = opponent_score, player_score

            return player_score, opponent_score, player_prev_score
                
    while (score0 < goal) and (score1 < goal):
        if who == 0:
            score0, score1, prev_score0 = one_turn(strategy0, score0, score1, prev_score0)
        else:
            score1, score0, prev_score1 = one_turn(strategy1, score1, score0, prev_score1)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        nonlocal tmp
        _map = {0: score0, 1: score1}
        if (_map[who] - last_score) > running_high:
            tmp = _map[who] - last_score
            print("{} point(s)! That's the biggest gain yet for Player {}".format(tmp, who))
        return announce_highest(who, _map[who], tmp)
    return say
    # END PROBLEM 7
...
def swap_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    attempt = score + free_bacon(opponent_score)
    if (is_swap(attempt, opponent_score)):
        if opponent_score >= attempt:
            return 0
        else:
            return num_rolls
    else:
