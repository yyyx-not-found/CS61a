...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        for i in range(self.min_range):
            ...
            if _place is beehive:
                return None

        step = self.min_range
        while _place is not beehive:
            res = rANTdom_else_none(_place.bees)
            if res != None:
                return res
            _place = _place.entrance
            step += 1
            if step > self.max_range:
                return None 
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if bee is not None and self.digesting == 0:
            self.digesting = self.time_to_digest + 1
            self.eat_bee(bee)
        # END Problem 6
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        "*** YOUR CODE HERE ***"
        super().__init__(armor)
        if not QueenAnt.is_used:
            QueenAnt.is_used = True
            self.is_true = True
        self.buffed = set()
        # END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.is_true:
            ...
            while _place is not None:
                if _place.ant is not None:
                    if isinstance(_place.ant, ContainerAnt):
                        # container
                        if _place.ant not in self.buffed:
                            self.buffed.add(_place.ant)
                            _place.ant.damage *= 2
                        # contained
                        if _place.ant.contained_ant is not None and _place.ant.contained_ant not in self.buffed:
                            self.buffed.add(_place.ant.contained_ant)
                            _place.ant.contained_ant.damage *= 2
                    else:
                        # general ants
                        if _place.ant not in self.buffed:
                            self.buffed.add(_place.ant)
