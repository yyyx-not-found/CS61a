...
def subseqs(s):
    ...
    if s == []:
        ...
        _next = subseqs(s[1:])
        return insert_into_all(s[0], _next) + _next
...
def trade(first, second):
    ...
    equal_prefix = lambda: reduce(add, first[:m]) == reduce(add, second[:n])
    while m < len(first) and n < len(second) and not equal_prefix():
        if reduce(add, first[:m]) < reduce(add, second[:n]):
            m += 1
        else:
            n += 1
...
def make_glookup(class_assignments):
    ...
    full_score = 0
    def helper(key, val):
        nonlocal score, full_score
        full_score += class_assignments[key]
        score += val
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    if n == 1 or n == 2:
        return 1
    return reduce(mul, [n - 1 + k for k in range(2, n)]) // reduce(mul, [k for k in range(2, n)])
    ...
    def counter():
        ...
        def helper(key):
            ...
            if key == 'reset':
                ...
                return local_counter[key]
            else:
                global_counter[key] = global_counter.get(key, 0) + 1
                return global_counter[key]
        return helper
    return counter
