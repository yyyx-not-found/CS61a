...
def num_splits(s, d):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(s, lst1, lst2):
        if not s:
            return 1 if abs(sum(lst1) - sum(lst2)) <= d else 0
        return helper(s[1:], lst1 + [s[0]], lst2) + helper(s[1:], lst1, lst2 + [s[0]])
    return helper(s, [], []) // 2
